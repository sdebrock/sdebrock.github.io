...
MyGame = ig.Game.extend({
    ...
    update: function() {
        ...
        if ( this.driving == true ) {
            var player = this.getEntitiesByType( EntityPorsche )[0];
            ig.game.gravity = 0;
        }
        else if ( this.driving == false ) {
            var player = this.getEntitiesByType( EntityPlayer )[0];
            ig.game.gravity = 300;
        }
    	if( player && this.driving == true) {
    		this.screen.x = player.pos.x - ig.system.width/2;
    		//if (this.screen.y < ig.system.height-176) {
                this.screen.y = player.pos.y - ig.system.height / 2;
            //}
            if(player.accel.x > 0 && this.instructText)
                this.instructText = null;
    	}else if( player && this.driving == false) {
            this.screen.x = player.pos.x - ig.system.width/2;
            if (this.screen.y < ig.system.height-176) {
            //this.screen.y = player.pos.y - ig.system.height / 2;
            }
...
// up by a factor of 2
ig.main( '#canvas', StartScreen, 30, 800, 608, 1 );
